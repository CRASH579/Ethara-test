...
export const Navbar = () => {
  ...
  const syncBubbles = useCallback(() => {
    ...
    if (desktopNav.current) {
      const activeDesktop = desktopNav.current.querySelector(
        ".nav-link.active",
      ) as HTMLElement;
    ...
    if (mobileNav.current) {
      const activeMobile = mobileNav.current.querySelector(
        ".mobile-link.active",
      ) as HTMLElement;
  ...
  return (
    <header className="fixed top-0 left-0 z-50 w-full">
      <div className="mx-auto max-w-7xl">
        <div className="hidden sm:flex mx-10 my-4 items-center justify-between py-10 px-3 bg-surface rounded-full">
          ...
          >
            ...
            {NAV_LINKS.map((link) => (
              <Link
                ...
                onMouseEnter={(e) =>
                  updateBubble(desktopNav.current, e.currentTarget, "hover")
                }
              >
                {link.label}
              </Link>
            ...
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Link
                  ...
                  onMouseEnter={(e) =>
                    updateBubble(mobileNav.current, e.currentTarget, "hover")
                  }
                >
                  <Icon
                    className={isActive(link.path) ? "text-light" : "text-text"}
                    size={22}
                  />
                </Link>
              );
